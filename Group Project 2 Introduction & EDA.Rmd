...
We will apply a Binomial Generalised Linear Model and we will initially consider the logit link. We assume this model since we have a binary response variable $Y_i$ and we have independent observations. There is some evidence of collinearity which we will ignore for now.
...
```{r wald, echo = FALSE, eval = TRUE,fig.align = "center", fig.cap = "\\label{fig:CI} The Wald Confidence Intervals for our log(odds) in our Full Model"}
...
```{r wald2, echo = FALSE, eval = TRUE,fig.align = "center"}
...
We see that flavor, aroma and acidity are still all significant and have similar coefficients. Our deviance however is quite high.

To explore other options we then fit a model with an interaction between flavor and aroma, since this makes intuitive sense.
...
Looking at the odds ratios of Model 1 we see
