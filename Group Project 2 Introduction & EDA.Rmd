...
summary.stat<-skim_without_charts(data14)%>%
  as.data.frame()%>%
  select(,-c(5:12,16:18))
summary.stat[is.na(summary.stat)]<-'-'
...
#build bar plots 
ggplot(top_5,aes(x=country_of_origin,y=counts,sort(y),fill='lightblue'))+
...
percentage<-na.omit(data14.year)%>% 
  tabyl(harvested,Qualityclass) %>% 
  adorn_percentages() %>% 
  adorn_pct_formatting() %>% 
  adorn_ns()

ggplot(na.omit(data14.year), aes(x= harvested,  y = ..prop.., group=Qualityclass, fill=Qualityclass)) + 
