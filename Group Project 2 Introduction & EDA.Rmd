---
title: "Analysis of the coffee beans"
author: "yifan & pearse"
date: "2021/7/12"
output:
...
Initially, we would like to explore the relationship between countries and quality class of coffee beans. Because the categories of origin country are too large and detailed, a column named *'continent'* was added to the tidy data with the goal of facilitating analysis(the source of the continent data has been represented on our github repository). Ignoring the good quality rate of the coffee beans is 100% as there is just one batch of coffee beans comes from Oceania. Figure \ref{fig:barplot1} shows that the coffee beans origin from Africa have the highest proportion of good quality in this sample(nearly 70%). Coffee beans from America and Asia are of similar quality.
...
Histograms are also added for checking the distributions of aroma,flavor and acidity grades.
...
We will apply a Binomial Generalised Linear Model and we will initially consider the logit link. We assume this model since we have a binary response variable $Y_i$ and we have independent observations. Figure \ref{fig:corr} suggests there is some evidence of collinearity which we will ignore for now.

From our Exploratory Data Analysis we expect Flavor, Aroma and Acidity to be good predictors of Qualityclass. We also have seen that Altitude and Category Two Defects may have some effect, so they are also included. 
...
The Wald Intervals for our first 3 variables do not contain 0 so we will keep them in the model. Intervals for the variables category_two_defects and altitude_mean_meters both contain 0, further evidence that they are not significant terms. 

We drop them from the model and fit the following model,
...
confint.default(model_1) %>% #confint.default calculates wald confidence intervals since logistic regression is asymptotically normal 
    kable(caption = '\\label{tab:wald2} Wald Confidence Intervals for log odds in Model 1',
booktabs = TRUE, linesep = "") %>%
kable_styling(font_size = 10, latex_options = "hold_position")
```

We see that flavor, aroma and acidity are still all significant and have similar coefficients and effects to the previous model. Our AIC however is quite high. 

To explore other options we then fit a model with an interaction, interaction between flavor and aroma is a natural choice.
...
confint.default(model_2_int) %>% #confint.default calculates wald confidence intervals since logistic regression is asymptotically normal 
  kable(caption = '\\label{tab:wald_int} Wald Confidence Intervals for log odds in Model 2',
booktabs = TRUE, linesep = "") %>%
kable_styling(font_size = 10, latex_options = "hold_position")
```
...
Figure \ref{fig:hist} suggests that flavor, aroma and acidity follow an approximately normal distribution, so we will fit a model with the probit link function. 
...
confint.default(model_3_probit) %>% #confint.default calculates wald confidence intervals since logistic regression is asymptotically normal 
  kable(caption = '\\label{tab:wald_prob} Wald Confidence Intervals for log odds in Model 3',
booktabs = TRUE, linesep = "") %>%
kable_styling(font_size = 10, latex_options = "hold_position")
```

All terms are significant as in the other models, however both AIC and Deviance are higher than in the others. We follow this with the cloglog model.
...
confint.default(model_4_cloglog) %>% #confint.default calculates wald confidence intervals since logistic regression is asymptotically normal 
  kable(caption = '\\label{tab:wald_clog} Wald Confidence Intervals for log odds in Model 4',
booktabs = TRUE, linesep = "") %>%
kable_styling(font_size = 10, latex_options = "hold_position")
```

Here again we find all terms significant and a low AIC and Deviance relative to the other models.
...
Models <- c("Model 1","Model 2 (Interaction)","Model 3 (Probit)", "Model 4 (cloglog)")
...
Looking at the odds ratios of Model 1 we have
...
With corresponding 95% Wald Confidence Intervals

```{r wald5, echo = FALSE, eval = TRUE}
exp(confint.default(model_1)) %>% #confint.default calculates wald confidence intervals since logistic regression is asymptotically normal 
    kable(caption = '\\label{tab:wald1} Wald Confidence Intervals for Odds Ratios of Model 1',
booktabs = TRUE, linesep = "") %>%
kable_styling(font_size = 10, latex_options = "hold_position")
```
...
We now plot the predicited probabilities of classification for each of the variable in our model

```{r plot_flav, echo = FALSE, eval = TRUE, fig.cap = "\\label{fig:Pred.flav} Predicted Probabilities of being classified as QualityClass Poor for Flavor Variable", fig.align="center",fig.pos="H"}
plot_model(model_1,type="pred")
```
Figure 8 shows us the predicted probabilities of being classified as Poor for values of Flavor, we see anything less than a 7 is classified poor and anything greater than 8 is classified as Good. The general trend is that which we have already suggested, as Flavor increases the probability of being classified as Poor decreases.




Figure 9 shows us the predicted probabilities of being classified as Poor for values of Aroma, we see anything less than a 6.5 is classified poor, however we do not see as distinct tails as in Figure 8 and cannot give such a distinct rule for being classified as good. The general trend is that which we have already suggested, as Aroma increases the probability of being classified as Poor decreases.



Figure 10 shows us the predicted probabilities of being classified as Poor for values of Aroma, we see anything less than a 6 is classified poor, however we do not see as distinct tails as in Figure 8 and cannot give such a distinct rule for being classified as good. The general trend is that which we have already suggested, as Aroma increases the probability of being classified as Poor decreases.

# Conclusion {#sec:Conc}

In conclusion we have found that increasing Flavor, Aroma and Acidity in coffee beans significantly reduces the chances of being classified as Poor Quality. Whilst Altitude and Number of Category 2 Defects have no significant effect. 

Flavour has the strongest effect on predicted probability of classification, with log odds coefficient `r model_1$coefficients[2]` visualised in Figure 8. 

Aroma has the second strongest effect, with log odds coefficient `r model_1$coefficients[3]` and Acidity the weakest, with log odds coefficient `r model_1$coefficients[4]`. Visualised in Figures 9 and 10 respectively.
